...
export default function IncidentDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(apiUrl(`/api/incidents/${params.id}`));
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(apiUrl(`/api/incidents/${incident!.id}`), { method: "DELETE" });
  ...
  async function handleAddUpdate() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(apiUrl(`/api/incidents/${incident!.id}/updates`), {
        method: "POST",
              ...
              return (
                <div key={u.id} className="relative flex gap-4 pb-6 last:pb-0">
                  <div className={`relative z-10 mt-1.5 size-3.5 shrink-0 rounded-full border-2 ${
                    u.status === "resolved" ? "border-success bg-success-light"
                    : u.status === "investigating" ? "border-warning bg-warning-light"
                    : "border-info bg-info-light"
                  }`} />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <Badge variant="light" color={uc.color} size="sm">{uc.label}</Badge>
                      <span className="text-xs text-text-soft">{new Date(u.createdAt).toLocaleString()}</span>
                    </div>
