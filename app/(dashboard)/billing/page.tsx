...
export default function BillingPage() {
  ...
  async function handleStripeCheckout() {
    setLoading("stripe");
    const res = await fetch(apiUrl("/api/billing/stripe/checkout"), { method: "POST" });
  ...
  async function handlePaystackCheckout() {
    setLoading("paystack");
    const res = await fetch(apiUrl("/api/billing/paystack/initialize"), { method: "POST" });
    const data = await res.json();
    if (data.url) window.location.href = data.url;
    setLoading(null);
  }
      ...
      <div className="rounded-lg border border-stroke-soft bg-bg-white p-5">
        ...
        {isPro ? null : (
          <div className="mt-4 flex gap-3">
            <Button variant="primary" icon={ArrowRight} loading={loading === "stripe"} onClick={handleStripeCheckout}>Upgrade with Stripe</Button>
            <Button variant="neutral" loading={loading === "paystack"} onClick={handlePaystackCheckout}>Pay with Paystack</Button>
          </div>
        )}
      </div>
    </div>
