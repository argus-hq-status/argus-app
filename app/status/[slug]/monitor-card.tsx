...
export function MonitorCard({
  ...
  name: string; url: string; status: string; avgPct: number; daysTracked: number; dailyHealth: DayData[];
}) {
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-3">
            ...
            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${PILL_COLORS[status] ?? PILL_COLORS.unknown}`}>
        ...
        <div className="mt-5 rounded-xl border border-gray-100 bg-gray-50 p-4">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400">{activeDay.successCount}/{activeDay.total} up</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-400">
              <span className="flex items-center gap-1"><span className="size-2 rounded-sm bg-green-400" />{activeDay.successCount}</span>
              {activeDay.failedCount > 0 && (
                <span className="flex items-center gap-1"><span className="size-2 rounded-sm bg-red-400" />{activeDay.failedCount}</span>
              )}
            </div>
          </div>
        </div>
