...
var t = `calc(${s} * 2)`;
var p$1 = "var(--number-flow-mask-width, 0.5em)";
var n = `calc(${p$1} / var(--scale-x))`;
...
var b$1 = (e) => `<span class="${e.type === "integer" || e.type === "fraction" ? "digit" : "symbol"}" part="${e.type === "integer" || e.type === "fraction" ? `digit ${e.type}-digit` : `symbol ${e.type}`}">${e.value}</span>`;
var i = (e, a) => `<span part="${a}">${e.reduce((l, f) => l + b$1(f), "")}</span>`;
var $ = (e = "") => m`:where(number-flow${e}){line-height:1}number-flow${e} > span{font-kerning:none;display:inline-block;padding:${t} 0}`;
var V$1 = (e, { nonce: a, elementSuffix: l } = {}) => h`<template shadowroot="open" shadowrootmode="open"
	...
	*/
	set data(t) {
		...
		if (this.created) {
			...
			this.nonce && (c.nonce = this.nonce), c.textContent = x$1, this.shadowRoot.appendChild(c), this._pre = new E(this, e, {
				justify: "right",
				part: "left"
			}), this.shadowRoot.appendChild(this._pre.el), this._num = new F(this, i, s), this.shadowRoot.appendChild(this._num.el), this._post = new E(this, o, {
				justify: "left",
				part: "right"
...
var isReact19 = parseInt("19.2.7".match(/^(\d+)\./)?.[1]) >= 19;
...
var NumberFlowImpl = class extends import_react.Component {
	...
	render() {
		...
		return /*#__PURE__*/ import_react.createElement("number-flow-react", {
			...
			nonce,
			...rest,
			dangerouslySetInnerHTML: { __html: V$1(data, {
				nonce,
				elementSuffix: "-react"
			}) },
